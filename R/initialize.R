...
initialize <- function(f, npar, minimize, controls) {
  ...
  y <- stats::runif(controls$init_runs * npar, controls$init_min, controls$init_max)
  ...
  for (n in seq_len(controls$init_runs)) {
    ...
    ### save local search
    local_searches[[n]] <- list(
      "success" = local_search$converged,
    ...
    if (local_searches[[n]]$success) {
      cat(" [found optimum]")
      if (unique_optimum(
        L = L, argument = local_searches[[n]]$argument,
